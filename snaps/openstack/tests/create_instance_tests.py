...
class CreateInstanceSingleNetworkTests(OSIntegrationTestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            # Create Flavor
            flavor_config = openstack_tests.get_flavor_config(
                name=guid + '-flavor-name', ram=256, disk=10, vcpus=2,
...
class CreateInstanceOnComputeHost(OSIntegrationTestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            # Create Flavor
            flavor_config = openstack_tests.get_flavor_config(
                name=guid + '-flavor-name', ram=512, disk=1, vcpus=1,
                metadata=self.flavor_metadata)
            self.flavor_creator = OpenStackFlavor(self.admin_os_creds,
                                                  flavor_config)
            self.flavor_creator.create()
...
class CreateInstanceTwoNetTests(OSIntegrationTestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            flavor_config = openstack_tests.get_flavor_config(
                name=self.guid + '-flavor-name', ram=512, disk=10, vcpus=2,
                metadata=self.flavor_metadata)
            self.flavor_creator = OpenStackFlavor(self.admin_os_creds,
                                                  flavor_config)
            self.flavor_creator.create()
...
class CreateInstanceVolumeTests(OSIntegrationTestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            # Create Flavor
            flavor_config = openstack_tests.get_flavor_config(
                name=guid + '-flavor-name', ram=256, disk=1, vcpus=2,
                metadata=self.flavor_metadata)
            self.flavor_creator = OpenStackFlavor(self.admin_os_creds,
                                                  flavor_config)
            self.flavor_creator.create()
