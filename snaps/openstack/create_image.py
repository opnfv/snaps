...
class OpenStackImage:
    ...
    def create(self, cleanup=False):
        ...
        if self.__image:
            ...
            if self.image_settings.extra_properties:
                extra_properties = self.image_settings.extra_properties
            else:
    ...
    def clean(self):
        ...
        if self.__kernel_image:
            try:
                glance_utils.delete_image(self.__glance, self.__kernel_image)
            except HTTPNotFound:
                pass
            self.__kernel_image = None

        if self.__ramdisk_image:
            try:
                glance_utils.delete_image(self.__glance, self.__ramdisk_image)
            except HTTPNotFound:
                pass
            self.__ramdisk_image = None
